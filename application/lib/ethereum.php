...
class Ethereum extends JSON_RPC
{
	...
	public function fromWei($numeric,$abs=8)
	{
            if(is_numeric($numeric)) return bcdiv($numeric,"1000000000000000000",$abs); else return FALSE;
	}
        function toWei($numeric)
	{
            if(is_numeric($numeric)) return bcmul($numeric,1000000000000000000); else return FALSE;
	}
	...
	public function decode_hex($input)
	{
		...
		if(preg_match('/[a-f0-9]+/', $input))
			return convert_dechex($input,16,10);
	...
	function eth_getBalance($address, $block='latest', $decode_hex=FALSE)
	{
		...
		if($decode_hex)
			$balance = $this->fromWei($this->decode_hex($balance));
	...
	function eth_sign($address, $input)
	{
        ...
        function personal_unlockAccount($address, $passphrase, $duration)
	{
		return $this->ether_request(__FUNCTION__, array($address, $passphrase, $duration));
	}
        
        function personal_sendTransaction($transaction, $passphrase)
	{
		return $this->ether_request(__FUNCTION__, array($transaction->toObject(), $passphrase));
	}
	...
	function eth_call($message, $block='')
	{
		...
		{
			return $this->ether_request(__FUNCTION__, array($message->toArray(),$block));
		}
	}
...
class Ethereum_Message
{
	...
	function __construct($to,$data,$value)
	{
            $this->to = $to;
            $this->value = $value;
            $this->data = $data;
